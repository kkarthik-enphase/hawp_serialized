...
class HAFMencoder(object):
    ...
    def _process_per_image(self,ann):
        ...
        lines_reshaped = lines.view(-1)
        num = lines.shape[0]
        start = time.time()

        print(num,height,width)
        for index in range(height * int(width/8)):
            if (index%1000 == 0):
                print(index,'t:', round(time.time() - start), 's')
            w = index % width
            ...
            for i in range(num):
                ...
                x1 = lines_reshaped[4 * i] * xs
                y1 = lines_reshaped[4 * i + 1] * ys
                x2 = lines_reshaped[4 * i + 2] * xs
                y2 = lines_reshaped[4 * i + 3] * ys
                dx = x2 - x1
                dy = y2 - y1
                ux = x1 - px
                uy = y1 - py
        ...
        lmap = lmap.reshape(-1,height,width)
